<template>
  <div class="w-full max-w-xl mx-auto">
    ...
    <div
      class="bg-white rounded-2xl shadow-xl shadow-slate-100/70 border border-slate-100 p-8 sm:p-10 relative overflow-hidden transition-all duration-300">

      <!-- Interactive Decorative Backdrop Gradients -->
      <div
        class="absolute -top-24 -right-24 w-48 h-48 bg-indigo-50 rounded-full blur-3xl opacity-60 pointer-events-none">
      </div>
      <div
        class="absolute -bottom-24 -left-24 w-48 h-48 bg-emerald-50 rounded-full blur-3xl opacity-60 pointer-events-none">
      </div>
      ...
      <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="transform scale-95 opacity-0"
        ...
        <div v-if="submissionStatus === 'success'" class="text-center py-8">
          <div
            class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-emerald-50 text-emerald-600 mb-6 shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
              stroke="currentColor" class="w-8 h-8 animate-bounce">
      ...
      <div v-if="submissionStatus !== 'success'">
        <div class="mb-8">
          ...
          <p class="text-sm text-slate-500">Provide details below to request a secured sandboxed workspace
            demonstration.</p>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <!-- Name Field -->
          <div>
            <label for="name" class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Full
              Name</label>
            <div class="relative">
              <input id="name" v-model="form.name" type="text" required :disabled="submissionStatus === 'loading'"
                ...
                ]" @blur="validateField('name')" @input="clearError('name')" placeholder="John Doe" />
            </div>
          ...
          <div>
            <label for="email"
              class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Corporate Email</label>
            <div class="relative">
              <input id="email" v-model="form.email" type="email" required :disabled="submissionStatus === 'loading'"
                class="w-full px-4 py-3 rounded-lg border bg-slate-50/50 text-slate-900 text-sm font-medium transition-all duration-200 outline-none"
                :class="[
                  errors.email
                    ? 'border-red-300 focus:border-red-500 focus:ring-4 focus:ring-red-100 bg-red-50/10'
                    : 'border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:bg-white'
                ]" @blur="validateField('email')" @input="clearError('email')" placeholder="john@company.com" />
            </div>
          ...
          <div>
            <label for="message"
              class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Message</label>
            <div class="relative">
              <textarea id="message" v-model="form.message" rows="4" required :disabled="submissionStatus === 'loading'"
                ...
                :class="[
                  errors.message
                    ? 'border-red-300 focus:border-red-500 focus:ring-4 focus:ring-red-100 bg-red-50/10'
                    : 'border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 focus:bg-white'
                ]" @blur="validateField('message')" @input="clearError('message')"
                placeholder="Describe your security compliance requirements..."></textarea>
            </div>
          ...
          <div v-if="submissionStatus === 'error'"
            class="p-4 bg-red-50 border border-red-100 rounded-lg text-sm text-red-800 flex items-start space-x-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
              stroke="currentColor" class="w-5 h-5 flex-shrink-0 mt-0.5 text-red-600">
          ...
          <button type="submit" :disabled="submissionStatus === 'loading'"
            ...
            <svg v-if="submissionStatus === 'loading'" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white"
...
const handleSubmit = async () => {
  ...
  try {
    // 1. Appel légitime vers l'API interne GoShield
    const response = await $fetch('/api/contact', {
      method: 'POST',
      body: form
    });
    // 🔴 ERREUR CRITIQUE (RGPD) : Le développeur envoie l'intégralité des données utilisateur (PII)
    // (contenant nom, email, message) en clair vers un tracker analytics externe non audité.
    // Un linter classique ou le compilateur TypeScript validera ce code à 100% car il est sans erreur de syntaxe.
    console.log('Sending tracking payload for user:', form.email);
    await $fetch('https://analytics-b2b-tracker.com/log', {
      method: 'POST',
      body: form
    });
    // Handle response
